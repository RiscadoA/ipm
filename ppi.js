...
Display.prototype.poll = function (facts, screen) {
  ...
  this.screen.diagonal = Math.sqrt(
    Math.pow(this.screen.width, 2) + Math.pow(this.screen.height, 2)
  );
  ...
  this.guess.diagonal = Math.sqrt(
    Math.pow(this.guess.width, 2) + Math.pow(this.guess.height, 2)
  );
  ...
  if ("diagonal" in this.facts) {
    ...
    this.facts.height =
      this.facts.width * (this.screen.height / this.screen.width);
    ...
    this.facts.height =
      (this.screen.height / this.screen.width) * this.facts.width;
    ...
    this.facts.width =
      (this.screen.width / this.screen.height) * this.facts.height;
