...
function drawUserIDScreen() {
  ...
  student_ID_form = createInput("99316"); // create input field
  ...
  display_size_form = createInput("17.3"); // create input field
...
function validSize() {
  if (
    ...
    parseInt(display_size_form.value()) > 10
  )
    return true;
  else {
...
function randomizeTrials() {
  for (
    var i = 0;
    i < 18;
    i++ // 4 rows times 4 columns = 16 targets
  )
    for (
      var k = 0;
      k < 3;
      k++ // each target will repeat 3 times (16 times 3 = 48 trials)
    )
      trials.push(i);
